...
int main() {
  iqsdk::TensorNetworkConfig tensor_config(total_qubits);
  iqsdk::TensorNetworkSimulator quantum_8086;
  ...
  quantum_8086.initialize(tensor_config);
  if (quantum_8086.isValid()) {
  ...
  iqsdk::QssMap<unsigned int> distribution =
      iqsdk::TensorNetworkSimulator::samplesToHistogram(measurement_samples);
  ...
  std::cout << std::endl << "testing . . .  getAmplitudes - vec" << std::endl;
  std::vector<std::complex<double>> amplitudes =
      quantum_8086.getAmplitudes(qids);
  for (std::vector<std::reference_wrapper<qbit>>::size_type i = 0;
       i != qids.size(); ++i) {
    std::cout << "qubit " << i << " " << amplitudes.at(i) << std::endl;
  }

  std::cout << std::endl << "testing . . .  displayAmplitudes" << std::endl;
  iqsdk::TensorNetworkSimulator::displayAmplitudes(amplitudes, qids);

  std::cout << std::endl
            << "testing . . .  getAmplitudes - QssMap" << std::endl;
  iqsdk::QssMap<std::complex<double>> amplitude_map =
      quantum_8086.getAmplitudes(qids, bases);
  for (const auto basis : amplitude_map) {
